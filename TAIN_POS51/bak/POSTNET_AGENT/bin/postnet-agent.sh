...
MAIN_CLASS="soaway.agent.event.LAgentInvoker"

if [[ `ps -ef | grep PostnetAgent | grep -v grep | grep -v tail | wc -l` -eq 0 ]];
    then
        echo $CLASS_PATH
        $JAVA_HOME/bin/java -Dname=PostnetAgent -cp $CLASSPATH -Dlaf.home=$LAF_HOME -Xms512m -Xmx512m -Dhttp.proxyHost=127.0.0.1 -Dhttp.proxyPort=8888 $MAIN_CLASS &
    else
        echo '#####################################################################'
        echo '#  [postnet-agent].... Process is already running.                  #'
        echo '#####################################################################'
fi
