...
public enum POSHW000003H {
	 ...
	 *
	 * <PRE>
	 *   -. ClassName  : POSHW000003H
	 *   -. MethodName : POSHW000003H
	 *   -. Comment    :
	 *   -. Author     : taincokr
	 ...
	 * @param defVal
	 */
	POSHW000003H(char type, char lead, int off, int len, String name, String desc, String defVal) {
		this.type = type;
		this.lead = lead;
		this.off = off;
		this.len = len;
	...
	private static int setLenTotal() throws Exception {
		...
		if (lenTotal < 0) {
			...
			for (POSHW000003H fld : POSHW000003H.values()) {
				off += fld.getLen();
				cnt ++;
			}
	...
	public static byte[] makeBytes() throws Exception {
		...
		if (flag) {
			POSHW000003H.DATA_CLAS         .setVal(bytes, "HD");
			POSHW000003H.SND_DATE          .setVal(bytes, new SimpleDateFormat("yyyyMMdd", Locale.KOREA).format(new Date()));
			POSHW000003H.HD_FILLER         .setVal(bytes, "");
		}
	...
	public static String decompress(byte[] bytes) throws Exception {
		...
		if (flag) {
			 ...
			 * 
			 */
			ret = POSHW000003H.makeBytes();
			
			String[] items = new String(bytes).split("\\|", POSHW000003H.getCntFld());   // TODO 2016.03.14 : have to fix the field count... ^^
			int i = 0;
	...
	private static final Logger log = Logger.getLogger(POSHW000003H.class);
	...
	private static void test01(String[] args) throws Exception {
		...
		if (flag) {
			...
			if (flag) log.debug(String.format("[FLD_CNT:%d] [REC_LEN:%d]", POSHW000003H.getCntFld(), POSHW000003H.getLength()));
			
			byte[] bytes = POSHW000003H.makeBytes();
			...
			POSHW000003H.print();
			POSHW000003H.print(bytes);
			
			String strCompress = POSHW000003H.compress(bytes);
			if (flag) log.debug("> COMPRESS [" + strCompress + "]");
			
			String strDecompress = POSHW000003H.decompress(strCompress.getBytes("EUC-KR"));
